...
import { Send, Mic, Square, ImagePlus, FileText, Plus, Clock, Download, LogOut, X, MapPin, ChevronDown, ChevronUp, CheckCircle, XCircle, Home } from 'lucide-react'
...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    if (navigator.geolocation) navigator.geolocation.getCurrentPosition(
      p => { window._lat = p.coords.latitude; window._lng = p.coords.longitude }, () => {})
  }, [])

  const fresh = async () => {
    try {
      const { data } = await createSession()
      setSid(data.id); setMsgs([]); setDiag(null); setHosps([]); setPopup(false); setFb('pending')
      nav('/chat/' + data.id, {replace: true})
    } catch {}
  }
  const load = async id => { try { const { data } = await getSession(id); setMsgs(data.messages || []) } catch {} }

  const handleResult = data => {
    if (data.diagnosis) { setDiag(data.diagnosis); setHosps(data.hospitals || []); setPopup(true); setFb('ask') }
  }
  ...
  const send = async () => {
    ...
    try {
      const b = {content: t}; if (window._lat) { b.lat = window._lat; b.lng = window._lng }
      const { data } = await sendMessage(sid, b)
      setMsgs(m => [...m, {role:'assistant', content:data.message, id:Date.now()+1}])
      handleResult(data)
    } catch { setMsgs(m => [...m, {role:'assistant', content:'Something went wrong. Please try again.', id:Date.now()+1}]) }
    setLoading(false)
  }
  ...
  const fbOk = async () => { try { await submitFeedback({prediction:diag.prediction_id, is_correct:true}); toast.success('Thank you'); setFb('done') } catch {} }
  const fbNo = () => setFb('wrong')
  const fbFix = async () => { try { await submitFeedback({prediction:diag.prediction_id, is_correct:false, correct_disease:corrDis||'Not specified'}); toast.success('Correction saved'); setFb('done'); setCorrDis('') } catch {} }
  ...
  return (
    <div className="flex h-screen" style={{background:'#f6f9f7'}}>
      ...
      <div className="w-56 text-white p-4 flex-col hidden md:flex" style={{background:'#0c1f17'}}>
        ...
        <button onClick={fresh} className="w-full p-2.5 rounded-lg flex items-center gap-2 text-sm font-semibold mb-2 shadow-lg text-white" style={{background:grad}}>
          <Plus size={15}/>New Consultation
        </button>
        <button onClick={()=>nav('/history')} className="w-full p-2.5 text-emerald-400/60 hover:text-white text-sm flex items-center gap-2 rounded-lg hover:bg-white/5">
      ...
      <div className="flex-1 flex flex-col">
        <header className="bg-white/80 backdrop-blur border-b border-gray-200/50 px-4 md:px-6 py-3 flex items-center justify-between">
          ...
          <div className="flex gap-1.5">
            {diag && <button onClick={dlPdf} className="p-2 rounded-lg text-emerald-600 hover:bg-emerald-50"><Download size={16}/></button>}
        ...
        <div className="flex-1 overflow-y-auto px-4 md:px-6 py-4 space-y-3">
          {msgs.length === 0 && (
            <div className="text-center mt-16">
              ...
              <p className="text-gray-500 text-sm mb-8 max-w-sm mx-auto">Describe your symptoms, upload a medical report, or ask any health question.</p>
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={()=>setPopup(false)}>
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[88vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <div className="flex gap-2 mt-2">
                      ...
                      <span className="bg-amber-400/20 px-2.5 py-0.5 rounded-full text-xs">{diag.urgency || 'Consult doctor'}</span>
                    </div>
                  </div>
              ...
              <div className="p-5 space-y-2.5">
                <Sec t="Clinical Reasoning"><p className="text-gray-600 text-sm">{diag.explanation}</p>{diag.causal_analysis && <p className="text-gray-500 text-xs mt-2 bg-gray-50 p-2 rounded font-mono">{diag.causal_analysis}</p>}</Sec>
                <Sec t="Medicines">{sp(diag.medicines).map((m,i) => <p key={i} className="text-sm text-gray-600 py-0.5">- {m}</p>)}</Sec>
                <Sec t="Diet">{sp(diag.diet).map((m,i) => <p key={i} className="text-sm text-gray-600 py-0.5">- {m}</p>)}</Sec>
                <Sec t="Home Care">{sp(diag.remedies).map((m,i) => <p key={i} className="text-sm text-gray-600 py-0.5">- {m}</p>)}</Sec>
                <Sec t="Physical Activity">{sp(diag.exercise).map((m,i) => <p key={i} className="text-sm text-gray-600 py-0.5">- {m}</p>)}</Sec>
                <Sec t="Red Flags">{sp(diag.when_to_see_doctor).map((m,i) => <p key={i} className="text-sm text-gray-600 py-0.5">- {m}</p>)}</Sec>
                <Sec t="Specialist"><p className="text-sm text-gray-700 font-medium">{diag.specialist || 'General Practitioner'}</p></Sec>
                {diag.alternatives?.length > 0 && <Sec t="Differential Diagnoses"><div className="flex flex-wrap gap-1.5">{diag.alternatives.map((a,i) => <span key={i} className="bg-gray-100 px-2.5 py-1 rounded-full text-xs text-gray-600">{a.disease} ({(a.confidence*100).toFixed(0)}%)</span>)}</div></Sec>}
                {hosps.length > 0 && <Sec t={'Nearby Hospitals (' + hosps.length + ')'}>{hosps.slice(0,5).map((h,i) => <div key={i} className="flex items-start gap-2 p-2 bg-gray-50 rounded-lg mb-1"><MapPin size={12} className="text-red-400 mt-0.5 shrink-0"/><div><p className="text-sm font-medium text-gray-700">{h.name}</p>{h.lat && h.lng && <a href={'https://www.google.com/maps/dir/?api=1&destination='+h.lat+','+h.lng} target="_blank" rel="noopener" className="text-xs text-emerald-600 hover:underline">Directions</a>}</div></div>)}</Sec>}

                {fb === 'ask' && <div className="bg-emerald-50 rounded-xl p-4 border border-emerald-100"><p className="text-sm font-semibold text-gray-700 mb-2.5">Was this diagnosis accurate?</p><div className="flex gap-2"><button onClick={fbOk} className="flex-1 py-2.5 bg-emerald-600 text-white rounded-lg text-sm font-medium flex items-center justify-center gap-1.5 hover:bg-emerald-700"><CheckCircle size={15}/>Correct</button><button onClick={fbNo} className="flex-1 py-2.5 bg-gray-200 text-gray-700 rounded-lg text-sm font-medium flex items-center justify-center gap-1.5 hover:bg-gray-300"><XCircle size={15}/>Incorrect</button></div></div>}
                {fb === 'wrong' && <div className="bg-amber-50 rounded-xl p-4 border border-amber-100"><p className="text-sm font-semibold text-gray-700 mb-1">What was the correct condition?</p><p className="text-xs text-gray-500 mb-2">This helps Arogya learn and improve.</p><input value={corrDis} onChange={e=>setCorrDis(e.target.value)} placeholder="Enter correct disease..." className="w-full p-2.5 bg-white border border-amber-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-amber-300 mb-2"/><div className="flex gap-2"><button onClick={fbFix} className="flex-1 py-2 bg-amber-500 text-white rounded-lg text-sm font-medium">Submit</button><button onClick={fbFix} className="py-2 px-4 bg-gray-200 text-gray-600 rounded-lg text-sm">Skip</button></div></div>}
                {fb === 'done' && <div className="bg-emerald-50 rounded-lg p-3 text-center border border-emerald-100"><p className="text-sm text-emerald-700 font-medium">Feedback recorded. Arogya is learning.</p></div>}

                <div className="flex gap-2 pt-2">
                  <button onClick={dlPdf} className="flex-1 py-3 text-white rounded-xl text-sm font-semibold flex items-center justify-center gap-2 shadow-lg shadow-emerald-200 hover:opacity-90" style={{background:grad}}><Download size={15}/>Download Report</button>
                  <button onClick={()=>setPopup(false)} className="flex-1 py-3 bg-gray-100 text-gray-700 rounded-xl text-sm font-semibold hover:bg-gray-200">Close</button>
                </div>
                <p className="text-[10px] text-gray-400 text-center">AI-generated guidance. Always consult a qualified doctor.</p>
              </div>
            </div>
          </div>
        )}
        ...
        {/* Input Bar */}
        <div className="bg-white/80 backdrop-blur border-t border-gray-200/50 p-3 md:p-4">
          {rec && (
          ...
          <div className="max-w-2xl mx-auto flex items-center gap-2">
            ...
            <button onClick={()=>fRef.current?.click()} className="p-2 text-gray-400 hover:text-emerald-600 rounded-lg hover:bg-emerald-50 transition-colors" title="Upload image"><ImagePlus size={18}/></button>
            <button onClick={()=>pRef.current?.click()} className="p-2 text-gray-400 hover:text-emerald-600 rounded-lg hover:bg-emerald-50 transition-colors" title="Upload PDF"><FileText size={18}/></button>
            {!rec && <button onClick={startRec} className="p-2 text-gray-400 hover:text-emerald-600 rounded-lg hover:bg-emerald-50 transition-colors" title="Voice"><Mic size={18}/></button>}
            ...
            <button onClick={send} disabled={loading || !input.trim() || rec}
              className="p-2.5 text-white rounded-xl disabled:opacity-30 shadow-md shadow-emerald-200 hover:opacity-90 transition-opacity"
